...
func ProofInThePi() []AwesomeMathInformation {
	...
	str = fmt.Sprintf("'Jesus Christ' in Greek = 754.  The continuously repeating 83 digits found by dividing 1/754 adds up to: %d or the number of degrees in a circle.", jesusgreek_circle_total)
	html = append(html, AwesomeMathInformation{str})
...
func doawesomechemistrymath(inums importantnumbers) []AwesomeChemistryInformation {
	...
	if total == jgc {
		str = fmt.Sprintf("Jesus Christ in Greek (2368) and the first 37 elements (proton and neutron composite numbers) add up to: %d", total)
	} else {
...
func main() {
	...
	http.HandleFunc("/awesomemath1.html", func(w http.ResponseWriter, r *http.Request) {
		...
		html = append(html, AwesomeMathInformation{str})
		str = fmt.Sprintf("'JESUS' = 888 in Koine Greek. JESUS is a perfect multiple of 37. E.g. 24x37=888")
		html = append(html, AwesomeMathInformation{str})
		str = fmt.Sprintf("'CHRIST' = 1480 in Koine Greek.  CHRIST is also a perfect multiple of 37.  E.g. 40x37=1480")
		html = append(html, AwesomeMathInformation{str})
